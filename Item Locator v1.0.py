...
def search():
    ...
    def search_it():
        ...
        time.sleep(0.6)
        my_cursor.execute(
            'select count(search_name) from item_location where search_name="%s"' % string_formatter(item.get()))
        for i in my_cursor:
            for j in i:
                if j == 1:
                    my_cursor.execute(
                        'select actual_location from item_location where search_name="%s"' % string_formatter(
                            item.get()))
                    for t in my_cursor:
                        for h in t:
                            status.set(' ')
                            tmg.showinfo('Location', f'{item.get()} is in {h}')
                else:
                    status.set(' ')
                    tmg.showerror('Error', f'No such item as {item.get()} exists in our database.')
